...
|Mask(S2PP)| initializes a bitmask that is available through |OPTION_MASK_S2PP| which is attached to |rs6000_isa_flags| specified by |Var| and in parallel specifies a macro |TARGET_S2PP| that is set to |1|. \reference{ gcc internals options}
...
\end{lstlisting} 
This is used in |rs6000.c| only when it is used to calculate the maximum amount of registers needed to hold a value of a certain mode. \reference{http://www.delorie.com/gnu/docs/gcc/gccint\_120.html} 
...
The last string is only for documentary purposes.\reference{GCC int define_constraint}
...
\end{description}
If one wants to add own predicates, GCC offers a manuals entry \reference{GCC Internals Predicates}.
