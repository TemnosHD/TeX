...
This thesis mainly focuses on a processor that is an essential part of the \acfi{HICANN-DLS}.
The following chapter will deal with the \ac{HICANN-DLS} as a whole and then look into the \acfi{PPU} in detail while also addressing processor architecture in general.
...
A \acfi{FPGA} is connected to all pre-synaptic inputs and routes external spikes to these inputs.
...
The \ac{FPGA} sends a 6-bit address whenever it sends a spike to a pre-synaptic input which then is compared by each synapse to the addresses they hold themselves.
...
Along each column sits a \acfi{CADC} that converts the signal one neuron receives into digital data for analysis but can also be accessed by the \ac{PPU} similar to the synaptic array.
...
\begin{wrapfigure}{l}{0.6\textwidth}
    \centering
    \includegraphics[width=0.6\textwidth]{pictures/nux.png}
...
\begin{wrapfigure}[15]{r}{0.5\textwidth}
    \centering
        ...
        \vspace*{-10mm}
        \caption{\label{fig:processor} Structure of von-Neumann Architecture}
\end{wrapfigure}
...
Prominent parts of the operational section are the \acfi{ALU} and the \acfi{RF}.
...
\begin{description}
    \item[\acfi{GPR}] These registers can store values for various causes but in most cases are soon to be used by the ALU. Most registers on a processor are typically GPRs.
        Any register that is not a GPR is called a \textbf{special purpose register SPR}
    \item[\acfi{LR}] This register marks the jump point of function calls. After a function completes, the program jumps to the address in the link register.
    \item[\acfi{CR}] This register's value is set by an instruction that compares one or two values in GPRs. Its value can be a condition for some instructions if they are to be executed or not.
\end{description}        
...
%memory
Memory is often implemented as \acfi{RAM} which is both readable and writable.
It can be split into two popular types which are \acfi{SRAM} and \acfi{DRAM}.
...
An overview of different assembly mnemonics for the POWER7 ISA can be seen in the appendix section~\ref{sec:inlineasm}.
...
A different kind of extension are vector extensions that do the same as the FPU but for vectors instead of floats.
...
The s2pp \acfi{VE} of nux is the \ac{PPU}'s distinct feature that allows for \acfi{SIMD} operations on synaptic weights.
The \ac{VE} is only weakly coupled to the \acfi{GPP} of the \ac{PPU} nand both parts can operate in parallel while interaction is highly limited.
...
The \acfi{VRF} 32 new vector registers which are each 128-bit wide~\citep{AV:registers}.
...
It resembles most characteristics of the s2pp vector extension, like a similar VRF, and is already implemented in the PowerPC back-end of \ac{GCC}, but both \ac{VE}s also feature differences.

The s2pp \ac{VE} features a vector accumulator of 128 bits width and a \ac{VCR} which holds 3 bits for each half byte of the vector, making 96 bit in total.
...
This is usually done by a \acfi{MMU} or a \acfi{MC}.
...
The main feature of the \ac{PPU} though is access to the synaptic array which is offered through an extra bus.
...
On \ac{RISC} architectures this is especially important as load and store instructions can take a few hundred times more clock cycles than normal register instructions and pipelining depends mainly on the instruction sequence.
...
\begin{lstlisting}[caption= Exemplatory Assembly invkation, label=lst:inlineasm]
...
But \ac{GCC} also supports the idea of a cross-compiler which is the concept of compiling code on one machine but running the code on a different machine that is also based on a different architecture.
One must though build a version of \ac{GCC} locally for every back-end one wants to compile code for.
...
There is one major competitor though which stands besides \ac{GCC} as an open source compiler suite which is Clang that is part of the \ac{LLVM} (low level virtual machine).
...
However while one can argue in favor for either one, \ac{GCC} seems a little better suited for our application. \todo{reference \ac{LLVM} vs. \ac{GCC} on ARM, \ac{LLVM} vs. Gcc in EISC}
...
In our case I have chosen \ac{GCC} over \ac{LLVM} for two main reasons.
...
The other is that \ac{GCC} support existed to a minimum before I started this thesis and thus there was a point to start from.
This topic will be referred to later on in the discussion but for now a short motivation seems to be sufficient.

By now \ac{GCC} is a stable release version of 6.3 with version 7 in the works but we will use an older version which is used internally at the BrainScaleS project which is version 4.9.2.
...
We take a special interest in the PowerPC back-end of \ac{GCC} which is called rs/6000 for IBMs \ac{RISC} system/6000 architecture that is equivalent to POWER.
...
As of now reload is being replaced by LRA (local register allocator) but \ac{GCC} 4.9.2 is not impacted by this therefore we are stuck with reload indefinitely.

To address possible errors in reload later on we now get to know one form of \ac{IR} in \ac{GCC} that is Register Transfer Language (\ac{RTL}).

\subsubsection{Insn Definition and Register Transfer Language}
\label{sec:defineinsn}
\ac{RTL}, which is not to be mixed up with Register Transfer Level, is a form of \ac{IR} the back-end uses to generate machine code.
...
It is inspired by Lisp lists and thus we will need to take a look at those at last in before we take on the task of extending a \ac{GCC} back-end.

We do so in looking at one of the most fundamental \ac{RTL} statements first while explaining the each part at a time.
...
|define_insn| is an \ac{RTL} expression that generates an \ac{RTL} equivalent to a machine instruction.
One such instruction is called an insn (short for instruction) hand has several properties like a name, an \ac{RTL} template, a condition template, an output template and attributes. \cite{GCCint:defineinsn}
...
It is a C expression an must render |true| in order to allow the matching \ac{RTL} pattern to be applied.
...
The attentive reader might have noticed that only \ac{RTL} template is written in \ac{RTL}.
This is true but still do insn patterns belong into this section.
The \ac{RTL} not only is the most important part of an insn but we will hardly see \ac{RTL} outside from \ac{RTL} templates.
Still should \ac{RTL} be mentioned in its other form here as it is used for debugging purposes.

\unsure{keep everything that follows?}

\ac{RTL} can be split into two phases which are non-strict \ac{RTL} and strict \ac{RTL}.
...
An example of non-strict \ac{RTL} and strict \ac{RTL} of the same code can be seen in figure \add{examples of \ac{RTL} code}
