   ...
   \begin{tikzpicture}[style={circle,draw,fill=black,minimum size=15}, line width=.2mm,
       ...
       { 
           ...
           \foreach \y in {0,...,31} 
              ...
              \node[minimum height=0.25mm, inner sep=0, rectangle, minimum width=5mm, fill=red, label=right:{Row}] (row) at (.3*31 + 1 + 1,5) {};
              \node[minimum height=5mm, inner sep=0, rectangle, minimum width=0.25mm, fill=blue, label=right:{Column}] (column) at (.3*31 + 1 + 1,6) {};
              \node[black,fill=black,radius=0.5mm, minimum size=0, inner sep=0.5mm, label=right:{Synapse}] (synapse) at (.3*31 + 1 + 1,4) {};
   \end{tikzpicture}
